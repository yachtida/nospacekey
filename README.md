...
- **インライン予測(アルファ版・opt-in)** — 明示確定後の続きをローカルCPUで予測し、薄い文字で表示
...
設定の「変換エンジン」から専用モデルをダウンロードして有効にすると、明示確定の約300ms後に
最大16文字の続きを薄い文字で表示します。右矢印または End で受理、Esc で破棄できます。
既定では無効で、処理はローカルCPU上だけで行われます。モデルのダウンロードには約171MB必要です。
...
[v1.2.2-beta.16](https://github.com/yachtida/nospacekey/releases/tag/v1.2.2-beta.16) は、
アンインストールが長時間止まったように見える問題を修正したprereleaseです。
未公開beta.15の改善も含み、使用中ファイルを再起動後に削除する選択と、中断した削除の再開に対応します。
`nospacekey-setup-1.2.2-beta.16-devsigned.exe` は開発用の自己署名版です。
正式署名版ではなく、Windowsのアプリケーション制御によって利用できない場合があります。
導入・更新・実再起動を含む全受入は未完了です。詳しい変更点と制限はリリースノートをご覧ください。
...
beta.16では、使用中のファイルがある場合に再起動後の削除を選べます。Windowsを自動再起動することはありません。
開発用の差し替えや破損で旧版ファイルの所有確認ができない場合は、削除を中断します。
