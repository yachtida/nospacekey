...
use windows::core::{
    implement, Error, IUnknown, Interface, Ref, Result, BOOL, GUID, HRESULT, PCWSTR, PWSTR,
};
...
impl StoreState {
    ...
    pub fn set_selection(&self, start: i32, end: i32) {
        self.doc.set_selection(start, end);
    }
}
...
impl ITextStoreACP_Impl for HarnessTextStore_Impl {
    ...
    fn RequestLock(&self, dwlockflags: u32) -> Result<HRESULT> {
        ...
        hlog(&format!(
            "RequestLock flags={dwlockflags:#x} <<< ok={}",
            hr.is_ok()
        ));
        hr?; // 編集セッションのエラーを伝播
    ...
    fn GetSelection(
        ...
        unsafe {
            *pselection = TS_SELECTION_ACP {
                acpStart: s,
                acpEnd: e,
                style: TS_SELECTIONSTYLE::default(),
            };
            *pcfetched = 1;
        }
        Ok(())
    ...
    fn SetText(
        ...
        if !self.st.locked.get() {
            return Err(Error::from(TS_E_NOLOCK));
        }
        let len = self.st.doc.len();
    ...
    fn GetACPFromPoint(&self, _: u32, _: *const POINT, _: u32) -> Result<i32> {
        Err(Error::from(TS_E_INVALIDPOS))
    }
...
impl ITfContextOwnerCompositionSink_Impl for HarnessTextStore_Impl {
    fn OnStartComposition(&self, _pcomposition: Ref<ITfCompositionView>) -> Result<BOOL> {
        ...
        hlog(&format!(
            "OnStartComposition buf={} sel={:?}",
            self.st.doc.len(),
            self.st.doc.selection()
        ));
...
mod tests {
    ...
    fn insert_query_then_settext_roundtrip() {
        ...
        unsafe {
            acp.InsertTextAtSelection(TS_IAS_QUERYONLY, &q, &mut a, &mut b, &mut ch)
                .unwrap();
        }
        assert_eq!((a, b), (0, 0));
        assert_eq!(st.full(), "");
        ...
        unsafe {
            acp.GetSelection(0, &mut sel, &mut fetched).unwrap();
        }
        assert_eq!(fetched, 1);
        ...
        let want_end = [TS_SELECTION_ACP {
            acpStart: 4,
            acpEnd: 4,
            style: TS_SELECTIONSTYLE::default(),
        }];
        unsafe {
            acp.SetSelection(&want_end).unwrap();
        }
        let mut sel2 = [TS_SELECTION_ACP::default()];
        let mut fetched2 = 0u32;
        unsafe {
            acp.GetSelection(0, &mut sel2, &mut fetched2).unwrap();
        }
    ...
    fn insert_noquery_mutates_and_reports_change() {
        ...
        let t = u16s("あ"); // cch = 1
        unsafe {
            acp.InsertTextAtSelection(TS_IAS_NOQUERY, &t, &mut a, &mut b, &mut ch)
                .unwrap();
        }
        assert_eq!(st.full(), "あ");
