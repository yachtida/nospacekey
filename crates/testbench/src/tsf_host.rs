...
impl TsfHost {
    /// 現在のコンテキスト。属性 probe など read-only EditSession を
    /// 本物の TIP が描いた状態に対して走らせるために公開する。
    pub fn context(&self) -> &ITfContext { &self._ctx }
    /// ThreadMgr::Activate が返したクライアント id（RequestEditSession 用）。
    pub fn client_id(&self) -> u32 { self.tid }
}
