...
pub fn run() -> i32 {
    ...
    let cases: &[Case] = &[
        ...
        ("equal body changes target and repairs selection without SetText", |h| {
            ...
            h.store.reject_text.set(true);
            h.store.reject_selection.set(true);
            let pending = h.execute(&next, &session) == ApplyOutcome::TextAppliedRepairPending;
            h.store.reject_selection.set(false);
            Ok(pending && next.identity != first.identity
                && h.execute(&next, &session) == ApplyOutcome::Applied
                && h.store.text_writes.get() == writes && h.store.full() == "abcd")
        }),
        ("equal body updates converted reading and selected attributes", |h| {
            ...
            Ok(h.execute(&next, &session) == ApplyOutcome::Applied
                && h.store.text_writes.get() == writes
