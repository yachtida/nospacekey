...
pub struct ZenzaiRuntimeStatusDto {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub latency_live: Option<ZenzaiLatencyTierDto>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub latency_convert: Option<ZenzaiLatencyTierDto>,
}

/// Per-tier aggregated speed stats for the settings UI. Values are clamped
/// engine aggregates; a tier with any non-finite field is dropped entirely
/// rather than partially displayed.
#[derive(Clone, Debug, PartialEq, serde::Serialize)]
pub struct ZenzaiLatencyTierDto {
    pub count: u32,
    pub p50_ms: f64,
    pub p95_ms: f64,
    pub max_ms: f64,
...
/// Latency stats come from the engine over IPC, so the numbers get a defensive
/// clamp: non-finite milliseconds reject the whole tier (a half-garbage line
/// would mislead the very tuning decision it exists to support), and the
/// sample count is capped at the engine's window size.
const ZENZAI_LATENCY_MS_MAX: f64 = 60_000.0;
const ZENZAI_LATENCY_SAMPLE_MAX: u32 = 100;

fn sanitize_zenzai_latency(
    tier: Option<ipc::protocol::ZenzaiLatencyTier>,
) -> Option<ZenzaiLatencyTierDto> {
    tier.and_then(|t| {
        fn finite_ms(value: f64) -> Option<f64> {
            if value.is_finite() && value >= 0.0 {
                Some(value.min(ZENZAI_LATENCY_MS_MAX))
            } else {
                None
            }
        }
        Some(ZenzaiLatencyTierDto {
            count: t.count.min(ZENZAI_LATENCY_SAMPLE_MAX),
            p50_ms: finite_ms(t.p50_ms)?,
            p95_ms: finite_ms(t.p95_ms)?,
            max_ms: finite_ms(t.max_ms)?,
...
fn zenzai_status_from_response(
    ...
    latency_live: Option<ipc::protocol::ZenzaiLatencyTier>,
    latency_convert: Option<ipc::protocol::ZenzaiLatencyTier>,
    ...
    Ok(ZenzaiRuntimeStatusDto {
        ...
        device,
        reason,
        latency_live: sanitize_zenzai_latency(latency_live),
        latency_convert: sanitize_zenzai_latency(latency_convert),
    })
}
...
mod tests {
    ...
    #[test]
    fn zenzai_status_latency_tiers_pass_through_within_bounds() {
        let tier = ipc::protocol::ZenzaiLatencyTier {
            count: 100,
            p50_ms: 51.7,
            p95_ms: 62.6,
            max_ms: 72.2,
        ...
        let dto = zenzai_status_from_response(
            "gpu_active".into(),
            None,
            None,
            None,
        ...
        assert_eq!(dto.latency_convert, None);
    }

    #[test]
    fn zenzai_status_latency_tiers_are_clamped_or_dropped() {
        // Over-range milliseconds clamp instead of displaying absurd values.
        let over = ipc::protocol::ZenzaiLatencyTier {
            count: 500,
            p50_ms: 120_000.0,
            p95_ms: 120_000.0,
            max_ms: 120_000.0,
        ...
        let dto =
            zenzai_status_from_response("gpu_active".into(), None, None, None, Some(over), None)
                .unwrap();
        assert_eq!(dto.latency_live.as_ref().unwrap().p50_ms, 60_000.0);
        assert_eq!(dto.latency_live.as_ref().unwrap().count, 100);

        // A non-finite field drops the whole tier: a half-garbage line must
        // not feed the user's inference-limit tuning decision.
        let nan = ipc::protocol::ZenzaiLatencyTier {
            count: 10,
            p50_ms: f64::NAN,
            p95_ms: 5.0,
            max_ms: 5.0,
        ...
        let dto =
            zenzai_status_from_response("gpu_active".into(), None, None, None, Some(nan), None)
                .unwrap();
        assert_eq!(dto.latency_live, None);
    }
}
