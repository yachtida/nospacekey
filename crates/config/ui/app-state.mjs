...
/**
 * One-line latency summary for a tier ("ライブ変換" / "確定変換"). Returns
 * null when the tier is absent or malformed so the caller can hide the line —
 * an absent tier (engine not warmed up, pre-stats engine) is normal, not an
 * error worth showing.
 */
export function zenzaiLatencyLabel(tier, tierName) {
  if (!tier) return null;
  const { p50_ms, p95_ms, max_ms } = tier;
  if (![p50_ms, p95_ms, max_ms].every(Number.isFinite)) return null;
  const count = Number.isFinite(tier.count) ? Math.max(0, Math.trunc(tier.count)) : 0;
  const timeouts = Number.isFinite(tier.timeout_count)
    ? Math.max(0, Math.trunc(tier.timeout_count)) : 0;
  if (count === 0) {
    return timeouts > 0
      ? `${tierName}: まだ測定サンプルがありません（時間切れ ${timeouts}回）`
      : null;
  }
  const round = (value) => Math.round(value);
  let label = `${tierName} 直近${count}回: 中央 ${round(p50_ms)}ms / 95%が ${round(p95_ms)}ms以内 / ` +
    `最遅 ${round(max_ms)}ms / 時間切れ ${timeouts}回`;
  if (timeouts > 0) {
    label += " — 時間切れが発生しています。推論上限を下げると改善することがあります。";
  }
  return label;
}
