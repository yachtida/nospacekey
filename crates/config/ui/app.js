...
function renderZenzaiLatencyLines() {
  const liveEl = document.getElementById("zenzai-latency-live");
  const convertEl = document.getElementById("zenzai-latency-convert");
  if (!liveEl || !convertEl) return;
  // An absent tier is normal (engine without stats / not yet warmed up): the
  // line hides instead of showing a placeholder that looks like an error.
  const live = zenzaiLatencyLabel(zenzaiRuntimeStatus?.latency_live, "ライブ変換");
  const convert = zenzaiLatencyLabel(zenzaiRuntimeStatus?.latency_convert, "確定変換");
  liveEl.hidden = live === null;
  liveEl.textContent = live ?? "";
  convertEl.hidden = convert === null;
