...
test("Zenzai latency labels summarize a tier and hide when absent", () => {
  const tier = { count: 100, p50_ms: 50.5, p95_ms: 68.7, max_ms: 116.4, timeout_count: 0 };
  assert.equal(zenzaiLatencyLabel(tier, "ライブ変換"),
    "ライブ変換 直近100回: 中央 51ms / 95%が 69ms以内 / 最遅 116ms / 時間切れ 0回");
  const withTimeouts = { ...tier, timeout_count: 3 };
  assert.equal(zenzaiLatencyLabel(withTimeouts, "確定変換"),
    "確定変換 直近100回: 中央 51ms / 95%が 69ms以内 / 最遅 116ms / 時間切れ 3回" +
    " — 時間切れが発生しています。推論上限を下げると改善することがあります。");
  // Timeouts-only observation (no successful sample yet) still reports misses.
  assert.equal(zenzaiLatencyLabel({ count: 0, p50_ms: 0, p95_ms: 0, max_ms: 0, timeout_count: 2 },
    "ライブ変換"),
    "ライブ変換: まだ測定サンプルがありません（時間切れ 2回）");
  // Absent or malformed tiers hide the line entirely.
  assert.equal(zenzaiLatencyLabel(null, "ライブ変換"), null);
  assert.equal(zenzaiLatencyLabel({ count: 0, p50_ms: 0, p95_ms: 0, max_ms: 0, timeout_count: 0 },
    "ライブ変換"), null);
  assert.equal(zenzaiLatencyLabel(
    { count: 10, p50_ms: Number.NaN, p95_ms: 5, max_ms: 5, timeout_count: 0 },
    "ライブ変換"), null);
});
