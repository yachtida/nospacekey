...
/// The two things the outbox needs from an engine connection. Split off from
/// `VerifiedEngineClient` so the reconnect verdict in `deliver` can run
/// against scripted connections.
trait ReceiptConnection {
    fn engine_epoch(&self) -> &str;
    /// None is a transport failure (broken pipe / mismatched ack), not an
    /// engine verdict.
    fn send_receipt(&mut self, receipt: &CommitReceipt, deadline: Instant) -> Option<ReceiptStatus>;
}

impl ReceiptConnection for VerifiedEngineClient {
    fn engine_epoch(&self) -> &str {
        &self.learning_identity().engine_epoch
    }
    ...
    fn send_receipt(&mut self, receipt: &CommitReceipt, deadline: Instant) -> Option<ReceiptStatus> {
        ...
        match self.request_within(&Request::CommitReceipt(receipt.clone()), deadline) {
            Ok(Response::CommitReceiptAck { commit_id, status }) if commit_id == receipt.commit_id => {
                Some(status)
            }
            _ => None,
        }
...
struct EngineReceiptTransport<C: ReceiptConnection> {
    client: Option<C>,
    connect: Box<dyn FnMut(Duration, Instant) -> Option<C> + Send>,
}

impl EngineReceiptTransport<VerifiedEngineClient> {
    fn new(pipe: String) -> Self {
        Self {
            client: None,
            connect: Box::new(move |timeout, deadline| {
                EngineClient::connect_verified_to(&pipe, timeout, deadline).ok()
            }),
        }
    }
}

impl<C: ReceiptConnection> EngineReceiptTransport<C> {
    fn ensure_connected(&mut self, deadline: Instant) -> bool {
        ...
        self.client = (self.connect)(timeout, deadline);
        self.client.is_some()
    }
}

impl<C: ReceiptConnection> ReceiptTransport for EngineReceiptTransport<C> {
    fn deliver(&mut self, receipt: &CommitReceipt, deadline: Instant) -> DeliveryResult {
        if !self.ensure_connected(deadline) {
            return DeliveryResult::Retry;
        }
        // A cached handshake predates every engine swap since that connection
        // was established, so it may not condemn the receipt's epoch on its
        // own: a false EpochChanged poisons the live epoch's learning
        // watermark for the rest of this process's life (observed 2026-09-12:
        // same engine for two hours, one stale handshake, and every candidate
        // window dropped afterwards). Only a fresh handshake may.
        if self
            .client
            .as_ref()
            .is_some_and(|client| client.engine_epoch() != receipt.engine_epoch)
        {
            self.client = None;
            if !self.ensure_connected(deadline) {
                return DeliveryResult::Retry;
...
impl ReceiptOutbox {
    pub fn start(pipe: String) -> std::io::Result<Self> {
        Self::start_with(EngineReceiptTransport::new(pipe))
    }
            ...
            .position(|item| item.next_attempt <= now || item.expires <= now)
        {
            ...
            if let Some(message) = terminal {
                if let Some(message) = message {
                    ...
                    crate::text_service::tip_log(&format!(
                        "ev=receipt_delivery_failed reason={message:?} sequence={} epoch={}",
                        item.receipt.commit_id.sequence, item.receipt.engine_epoch
                    ));
...
mod tests {
    ...
    impl ReceiptConnection for ScriptedConnection {
        fn engine_epoch(&self) -> &str {
            self.epoch
        }
        fn send_receipt(
    ...
    fn scripted_transport(
        ...
        EngineReceiptTransport<ScriptedConnection>,
        Arc<AtomicUsize>,
    ) {
        let queue = Arc::new(Mutex::new(
            connections.into_iter().collect::<VecDeque<_>>(),
        ));
        let connects = Arc::new(AtomicUsize::new(0));
        let transport = {
            let queue = queue.clone();
            let connects = connects.clone();
            EngineReceiptTransport {
                client: None,
                connect: Box::new(move |_timeout, _deadline| {
    ...
    fn receipt_with_epoch(epoch: &str) -> CommitReceipt {
        let mut payload = receipt();
        payload.engine_epoch = epoch.into();
        payload
    }

    #[test]
    fn stale_cached_epoch_reconnects_before_declaring_epoch_changed() {
        // Pinned regression for the 2026-09-12 incident: a cached handshake
        // predating an engine swap must not condemn the live epoch. The stale
        // connection would fail any send, so the acknowledgement proves the
        // reconnected one served the receipt.
        let (mut transport, connects) = scripted_transport(vec![
            ScriptedConnection {
                epoch: "stale",
                status: None,
            },
            ScriptedConnection {
                epoch: "live",
                status: Some(ReceiptStatus::Applied),
            },
        ]);
        let deadline = Instant::now() + Duration::from_secs(2);
        assert_eq!(
            transport.deliver(&receipt_with_epoch("live"), deadline),
            DeliveryResult::Acknowledged(ReceiptStatus::Applied)
        );
        assert_eq!(connects.load(Ordering::Acquire), 2);
    }

    #[test]
    fn true_epoch_change_survives_a_fresh_handshake() {
        let (mut transport, connects) = scripted_transport(vec![
            ScriptedConnection {
                epoch: "stale",
                status: Some(ReceiptStatus::Applied),
    ...
    #[test]
    fn reconnect_failure_is_transient_not_epoch_changed() {
        let (mut transport, connects) = scripted_transport(vec![ScriptedConnection {
            epoch: "stale",
            status: Some(ReceiptStatus::Applied),
        }]);
        let deadline = Instant::now() + Duration::from_secs(2);
        // The second connect finds no engine; that says nothing about epochs.
        assert_eq!(
            transport.deliver(&receipt_with_epoch("live"), deadline),
            DeliveryResult::Retry
        );
    ...
    #[test]
    fn matching_epoch_sends_without_reconnecting() {
        let (mut transport, connects) = scripted_transport(vec![ScriptedConnection {
            epoch: "live",
            status: Some(ReceiptStatus::Applied),
        }]);
    ...
    #[test]
    fn send_failure_drops_the_connection_for_the_next_attempt() {
        let (mut transport, connects) = scripted_transport(vec![
            ScriptedConnection {
                epoch: "live",
                status: None,
            ...
            ScriptedConnection {
                epoch: "live",
                status: Some(ReceiptStatus::AlreadyProcessed),
            },
        ]);
        let deadline = Instant::now() + Duration::from_secs(2);
        assert_eq!(
    ...
    #[test]
    fn stale_handshake_does_not_poison_the_live_epoch_watermark() {
        // End-to-end pinned regression: the pre-fix transport returned
        // EpochChanged from the stale cache alone, and the resulting
        // rejected_generations["live"] = u64::MAX stripped the learning
        // identity from every later clause reply for the process's lifetime.
        let (transport, _connects) = scripted_transport(vec![
            ScriptedConnection {
                epoch: "stale",
                status: None,
            },
        ...
        let outbox = ReceiptOutbox::start_with(transport).unwrap();
        assert!(outbox.text_applied(receipt_with_epoch("live"), Instant::now()));
        let deadline = Instant::now() + Duration::from_secs(3);
        while outbox.outstanding.load(Ordering::Acquire) != 0 {
            assert!(Instant::now() < deadline, "delivery did not settle");
            std::thread::sleep(Duration::from_millis(5));
        }
        assert_eq!(outbox.take_notice(), None);
        assert!(!outbox.take_learning_invalidation());
        assert!(!outbox.rejects_identity(&ipc::client::EngineLearningIdentity {
            engine_epoch: "live".into(),
