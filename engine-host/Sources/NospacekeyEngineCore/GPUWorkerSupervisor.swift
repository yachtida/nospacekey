...
/// Sanitized per-tier speed stats for the settings UI.  Only successful
/// request timings feed the percentiles; budget misses are counted
/// separately so the two signals stay observable side by side.
public struct GPUWorkerLatencyTierSnapshot: Equatable, Sendable {
...
public struct GPUWorkerLatencySnapshot: Equatable, Sendable {
    public let live: GPUWorkerLatencyTierSnapshot?
    public let convert: GPUWorkerLatencyTierSnapshot?

    public init(live: GPUWorkerLatencyTierSnapshot?, convert: GPUWorkerLatencyTierSnapshot?) {
        self.live = live
...
/// path, input, candidate text, and generation.  Aggregated speed stats are
/// included because they carry no user content.
public struct GPUWorkerSupervisorSnapshot: Equatable, Sendable {
    public let state: GPUWorkerDisplayState
    public let backend: String?
    public let device: String?
    public let reason: String?
    public let latency: GPUWorkerLatencySnapshot?

    public init(state: GPUWorkerDisplayState,
                backend: String? = nil,
                device: String? = nil,
                reason: String? = nil,
                latency: GPUWorkerLatencySnapshot? = nil) {
        self.state = state
...
public final class GPUWorkerSupervisor: @unchecked Sendable {
    ...
    public func rerank(
        ...
        gpuEngineTraceLog(
            ...
            enabled: graceEnabled)
        let latencyOutcome: GPUWorkerLatencyTracker.Outcome
        switch reply {
        case .response: latencyOutcome = .success
        case .timeout: latencyOutcome = .timeout
        default: latencyOutcome = .other
        }
        latencyTracker.record(caller: caller, elapsedMs: elapsedMs, outcome: latencyOutcome) { [weak self] line in
            self?.logEvent(line)
        }
    ...
    private func snapshotLocked() -> GPUWorkerSupervisorSnapshot {
        let latency = GPUWorkerLatencySnapshot(
            live: latencyTracker.summary(isLive: true),
            convert: latencyTracker.summary(isLive: false))
        switch internalState {
        case .stopped:
        ...
        case .ready:
            return GPUWorkerSupervisorSnapshot(state: .gpuActive, backend: backend, device: device,
                                               latency: latency)
        case .quarantined:
            return GPUWorkerSupervisorSnapshot(state: .classic, backend: backend, device: device,
                                               reason: reason, latency: latency)
        case .disabled:
            return GPUWorkerSupervisorSnapshot(state: .disabled, latency: latency)
        }
    }
}

/// Per-tier latency window with a batched summary line and a queryable
/// snapshot for the settings UI.  Success timings are the evidence for (or
/// against) "the live budget is below the on-device P95", which single
/// failure logs can never show; timeouts are counted, not sampled, so a
/// budget miss never drags the percentiles up.
private final class GPUWorkerLatencyTracker: @unchecked Sendable {
    ...
    private let live = TierState()
    private let convert = TierState()

    private func state(for caller: GPUWorkerCaller) -> TierState {
        caller.isLiveTier ? live : convert
    }

    func record(caller: GPUWorkerCaller, elapsedMs: Double, outcome: Outcome,
                emit: (String) -> Void) {
        lock.lock()
        let tier = state(for: caller)
        var summaryToEmit: String?
        switch outcome {
        case .success:
            tier.samples.append(elapsedMs)
            if tier.samples.count > GPUWorkerSupervisor.latencySummaryBatch {
                tier.samples.removeFirst(tier.samples.count - GPUWorkerSupervisor.latencySummaryBatch)
            }
            tier.sinceLastEmit += 1
            if tier.sinceLastEmit >= GPUWorkerSupervisor.latencySummaryBatch {
                tier.sinceLastEmit = 0
                summaryToEmit = Self.summaryLine(tier: caller.isLiveTier ? "live" : "convert",
        ...
        lock.unlock()
        if let summaryToEmit {
            emit(summaryToEmit)
        }
    }

    func summary(isLive: Bool) -> GPUWorkerLatencyTierSnapshot? {
        lock.lock()
        defer { lock.unlock() }
        let tier = isLive ? live : convert
        guard !tier.samples.isEmpty || tier.timeoutCount > 0 else { return nil }
        let sorted = tier.samples.sorted()
        func percentile(_ fraction: Double) -> Double {
            let index = Int((Double(sorted.count) - 1) * fraction)
            return sorted[max(0, min(sorted.count - 1, index))]
        }
        return GPUWorkerLatencyTierSnapshot(
            sampleCount: sorted.count,
            p50Ms: sorted.isEmpty ? 0 : percentile(0.5),
            p95Ms: sorted.isEmpty ? 0 : percentile(0.95),
            maxMs: sorted.last ?? 0,
            timeoutCount: tier.timeoutCount)
    }
