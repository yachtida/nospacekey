...
final class GPUWorkerSupervisorTests: XCTestCase {
    ...
    // MARK: - Latency stats (settings UI)

    private func successReply(requestID: UInt64, generation: UInt64) -> GPUWorkerTransportReply {
        .response(GPUWorkerResponse(
            requestID: requestID, generation: generation,
            mainResults: ["A"], firstClauseResults: ["A"]))
    }

    func testLatencySnapshotCountsSuccessesAndTimeoutsSeparately() {
        let transport = FakeGPUWorkerTransport()
        transport.replies = [
            successReply(requestID: 1, generation: 1),
            successReply(requestID: 2, generation: 1),
        ]
        let supervisor = GPUWorkerSupervisor(transport: transport, retryCooldown: 60)

        _ = supervisor.rerank(classic: classic(), snapshot: snapshot(), leftContext: nil,
                              nBest: 10, inferenceLimit: 1, deadline: 1.2, caller: .live)
        _ = supervisor.rerank(classic: classic(), snapshot: snapshot(), leftContext: nil,
                              nBest: 10, inferenceLimit: 1, deadline: 1.2, caller: .live)
        transport.replies = []   // empty queue answers .timeout
        _ = supervisor.rerank(classic: classic(), snapshot: snapshot(), leftContext: nil,
                              nBest: 10, inferenceLimit: 1, deadline: 1.2, caller: .live)

        let latency = supervisor.snapshot.latency
        XCTAssertEqual(latency?.live?.sampleCount, 2,
                       "only successful requests feed the percentiles")
        XCTAssertEqual(latency?.live?.timeoutCount, 1,
                       "a budget miss is counted, never sampled into the percentiles")
        XCTAssertGreaterThan(latency?.live?.maxMs ?? 0, 0)
        XCTAssertNil(latency?.convert, "the untouched tier stays absent")
    }

    func testLatencySnapshotResetsOnModelOrRuntimeChangeAndExplicitRetry() {
        let transport = FakeGPUWorkerTransport()
        transport.replies = [successReply(requestID: 1, generation: 1)]
        let supervisor = GPUWorkerSupervisor(transport: transport, retryCooldown: 60)
        _ = supervisor.rerank(classic: classic(), snapshot: snapshot(), leftContext: nil,
                              nBest: 10, inferenceLimit: 1, deadline: 1.2, caller: .live)
        XCTAssertNotNil(supervisor.snapshot.latency?.live)

        // An inference-limit change travels as a model/runtime change: the
        // user is about to compare latencies across a new configuration, so
        // the previous window must not blend into the fresh one.
        supervisor.modelOrRuntimeChanged()
        XCTAssertNil(supervisor.snapshot.latency?.live,
                     "modelOrRuntimeChanged must reset the stats window")

        transport.replies = [successReply(requestID: 2, generation: 2)]
        supervisor.startWarmUp()
        XCTAssertTrue(waitUntil(supervisor.snapshot.state == .gpuActive))
        _ = supervisor.rerank(classic: classic(), snapshot: snapshot(), leftContext: nil,
                              nBest: 10, inferenceLimit: 1, deadline: 1.2, caller: .live)
        XCTAssertEqual(supervisor.snapshot.latency?.live?.sampleCount, 1)

        // explicitRetry is only meaningful in a degraded state (the UI button
        // exists for the classic latch); from .ready it is a documented no-op,
        // so verify the reset from a timeout-latched generation instead.
        transport.replies = []   // empty queue answers .timeout
        _ = supervisor.rerank(classic: classic(), snapshot: snapshot(), leftContext: nil,
                              nBest: 10, inferenceLimit: 1, deadline: 1.2, caller: .live)
        supervisor.explicitRetry()
        XCTAssertNil(supervisor.snapshot.latency?.live,
                     "explicit retry resets the stats window too")
    }

    func testOrdinaryReloadKeepsLatencyStats() {
        let transport = FakeGPUWorkerTransport()
        transport.replies = [successReply(requestID: 1, generation: 1)]
        let supervisor = GPUWorkerSupervisor(transport: transport, retryCooldown: 60)
        _ = supervisor.rerank(classic: classic(), snapshot: snapshot(), leftContext: nil,
        ...
        supervisor.ordinaryReload()
        XCTAssertEqual(supervisor.snapshot.latency?.live?.sampleCount, 1,
                       "an ordinary reload keeps the stats window, like the failure latch")
    }
